...
def _run_local(argv, timeout=3):
    """Run a command on the host Windows system for local metrics."""
    try:
...
def _collect_processes_wsl():
    ...
    stdout, _, rc = _run_wsl(['ps', 'aux', '--no-headers', '--sort=-%cpu'], timeout=4)
    procs = []
...
def _collect_containers_wsl():
    ...
    containers = []
    stdout, _, rc = _run_wsl(['docker', 'ps', '--format', '{{.Names}}|||{{.Status}}|||{{.Image}}'], timeout=4)
    if rc != 0 or not stdout:
        return containers
    for line in stdout.split('\n'):
        parts = line.split('|||')
    ...
    if op == 'liae_start':
        ...
        for s in selected:
            ...
            timeline.append({
                ...
                'cpu': round(top_cpu, 1),
                'mem': round(mem_pct, 1),
                'net': s.get('network', {}).get('established', 0),
                'ac':  len(anomalies),
                'hc':  1 if any(a.get('severity') == 'critical' for a in anomalies) else 0,
            })
